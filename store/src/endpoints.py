...
def getter(key):
    ...
    app.logger.info(f'causal context:{causal_context}\n\n')
    ...
    if key in kvs.state.storage:
        ...
        if entry['method'] == 'DELETE':
            return json.dumps({"does_exist":False,"error":"Key does not exist","message":"Error in GET", "address":kvs.state.address, 'causal-context':causal_context}), 404
        else:
            return json.dumps({"does_exist":True, "message":"Retrieved successfully", "value": entry['value'], "address":kvs.state.address, 'causal-context':causal_context}), 200 
    else:
        if key in queue and queue[key]['method'] !='DELETE':
            return json.dumps({"does_exist":True, "message":"Retrieved successfully", "value": queue[key]['value'], "address":kvs.state.address, 'causal-context':causal_context}), 200,
        return json.dumps({"does_exist":False,"error":"Key does not exist","message":"Error in GET", "address":kvs.state.address, 'causal-context':causal_context}), 404
...
def putter(key):
    ...
    for cc_key in queue.keys():
        causal = Entry.compare_entries(kvs.state.storage.get(cc_key, {}), queue[cc_key])
        if causal == constants.LESS_THAN:
            kvs.state.logical += 1
            if key not in kvs.state.storage: kvs.state.key_count += 1
            kvs.state.storage[cc_key] = Entry.max_of_entries(kvs.state.storage.get(cc_key, {}), queue[cc_key])
...
def deleter(key):
    ...
    if in_storage:
        ...
        return json.dumps({"does_exist": True, "message": "Deleted successfully"}), 200
    else:
        if key in queue and queue[key]['method'] != 'DELETE':
            return json.dumps({"does_exist": True, "message": "Deleted successfully"}), 200
        return json.dumps({"does_exist": False, "error": "Key does not exist", "message": "Error in DELETE"}), 404
...
def count():
    return json.dumps({"message":"Key count retrieved successfully","key_count":kvs.state.key_count, 
...
def get_shard_information(id):
    ...
    for i in range(kvs.state.repl_factor):
        ...
        key_count = max(key_count, response.json()['key_count'])
    return json.dumps({"message": "Shard information retrieved successfully", "shard-id": id, 
        "key_count": key_count, "replicas":kvs.state.view[shard_id*kvs.state.repl_factor:(shard_id+1)*kvs.state.repl_factor]}), 200
